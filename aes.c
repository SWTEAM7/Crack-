 ...
 * @param ctx AES 컨텍스트 포인터 (ctx->Nr이 미리 설정되어 있어야 함)
 * @param key 원본 키 (16/24/32 바이트)
 * @param keyLen 키 길이 (AES128/AES192/AES256)
 * @details FIPS-197 표준의 키 확장 알고리즘 구현
 *          순수한 알고리즘 함수: 이미 설정된 파라미터(ctx->Nr, keyLen)를 사용하여
 *          키 스케줄 알고리즘만 수행합니다.
 * 
 * @pre ctx->Nr이 AES_init에서 미리 설정되어 있어야 함
 * 
 * 알고리즘:
...
AESStatus AES_init(AES_ctx* ctx, const uint8_t* key, AESKeyLength keyLen){ /* AES 컨텍스트 초기화 */
    ...
    // ========================================================================
    // 1단계: 파라미터 설정 (초기화 함수에서 명시적으로 설정)
    // ========================================================================
    // 키 길이에 따른 라운드 수(Nr) 결정
    // - AES-128 (16바이트 키): 10라운드
    // - AES-192 (24바이트 키): 12라운드
    // - AES-256 (32바이트 키): 14라운드
    int Nk = (int)keyLen / 4;  // 키 워드 수
    ctx->Nr = (Nk == 4) ? 10 : ((Nk == 6) ? 12 : 14);
    
    // ========================================================================
    // 2단계: 키 확장 수행 (이미 설정된 파라미터를 기반으로 알고리즘만 수행)
    // ========================================================================
    key_expansion(ctx, key, keyLen);
    
    // ========================================================================
    // 3단계: 함수 포인터 및 에러 상태 초기화
    // ========================================================================
