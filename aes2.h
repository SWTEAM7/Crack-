 ...
 * [MAP]  2) 프로파일별 설계(AES2) 전반을 SHA-512로 확정 반영.
 * ==========================================================================*/
...
/* == 태그 길이(절단 길이) ==================================================
 * [PLAN]  AES2 태그 32B 명시(일부 섹션), 16/32B 운용 가능
...
typedef struct {
    ...
    size_t         salt_len;
    const uint8_t* info;   // [PLAN] 도메인 분리 문자열(예: "AES2|rk")
    size_t         info_len;
} AES2_KDFParams;
...
typedef enum {
    AES2_F_NONE        = 0,
    AES2_F_MAC_ENABLE  = 1 << 0,  // [PLAN] 무결성 태그 사용(EtM: HMAC-SHA-512)
    AES2_F_NONCE_GUARD = 1 << 1   // [PLAN] 최근 nonce/IV 재사용 차단
} AES2_Flags;

/* == 보안 컨텍스트 =========================================================
 * [PLAN]  AES 컨텍스트(aes_ctx) 존재, AES2에서 KDF로 라운드키 주입·고정
...
typedef struct {
    ...
    uint8_t       tag_len;       // [MAP] AES2_TagLen_16 또는 _32
 ...
 *   - AES2(보안형): HKDF-**SHA-512**로 파생된 키를 주입·고정(라운드키 독립성)
...
AESStatus AES2_init_hardened(AES2_SecCtx* s,
                             ...
                             AES2_Flags flags,
                             AES2_TagLen mac_tag_len /* 16 or 32 */);
 ...
 * [PLAN]  AES2-보안형 CBC 경로(패딩 필요, SHA-512)
 ...
 * [MAP]   내부에서 AES2_rand_bytes로 안전 생성 -> out_nonce/out_iv 반환.
...
/* == 라이브러리 정보/자가진단 =============================================
 * [PLAN]  selftest(KAT) 훅
 * [MAP]   AES2_selftest() 제공(NIST/RFC 벡터 기반). libinfo는 기능비트/버전 조회.
 */
typedef struct {
 ...
 */
#define AES2_TAGLEN_MIN 16
#define AES2_TAGLEN_MAX 32
